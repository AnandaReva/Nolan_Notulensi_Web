...
class MeetingController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('home', compact('meetings', 'participants', 'attendanceData', 'userRole', 'keyword', 'date', 'rejectionMessages', 'idLogin'));
    }
    ...
    public function store(Request $request, $idLogin)
    {
        ...
        if ($files) {
            foreach ($files as $file) {
                ...
                $url = $file->store('meetingFile', 'public');
    ...
    public function edit($id, $idLogin)
    {
      ...
      /*   Content::where('meeting_id', $meetingId)->delete(); //hapus semua content dan action dari meeting_id */
        ...
        if (is_array($editedDiscussionData)) {
            ...
            foreach ($separatedData as $data) {
                ...
                // Simpan data ke dalam tabel revision_history
                if (!empty($discussionContent)) {
                    // Simpan data ke dalam tabel revision_history
                    $revisionHistory = new Revision_History([
                        'meeting_id' => $meetingId,
                        'discussion_log' => json_encode(['content' => $discussionContent, 'action' => $actions]),
                        'editor' => $request->input('note_taker'),
                    ]);

                    $revisionHistory->save();
                }
            }
