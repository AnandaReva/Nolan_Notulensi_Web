                ...
                <tr data-discussion-index="{{ $index }}">
                    ...
                    <td class="text-start col-4">
                        <textarea id="discussion" cols="30" rows="4" class="form-control"
                            name="newDiscussion[{{ $index }}][content]">
                            {{ $content->discussion }}
                        </textarea>
                    </td>
                    <td class="text-start col-4" name="item[{{ $index }}][]" id="action-col-{{ $index }}">
                        @foreach ($content->actions as $idx => $action)
                            <input class="w-100 mb-3" id="actions" type="text"
                                name="newDiscussion[{{ $index }}][actions][{{ $idx }}][item]"
                                value="{{ $action->item }}">
                        @endforeach
                    </td>
                    <td class="col-1" name="due[0][]" id="due-date-col-{{ $index }}">
                        @foreach ($content->actions as $idx => $action)
                            <input class="mb-3" id="dueDate" type="date"
                                name="newDiscussion[{{ $index }}][actions][{{ $idx }}][due]"
                                value="{{ $action->due }}">
                        @endforeach
                    </td>
                    <td class="col-2">
                        <div id="pic-{{ $index }}">
                            @foreach ($content->actions as $idx => $action)
                                <select id="pic_0"
                                    name="newDiscussion[{{ $index }}][actions][{{ $idx }}][pic]">
                        ...
                        <button class="border-0 bg-transparent" id="add-action"
                            onclick="addNewAction(event, {{ $index }})"><i
...
@unless ($currentUrl == 'view')
    @if ($currentUrl == 'edit')
        <button onclick="addRow(event, 'edit')" class="btn btn-outline-dark p-2" id="addDiscussion"><i
